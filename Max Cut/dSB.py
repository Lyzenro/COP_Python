...
class DiscreteSimulatedBifurcation:
    ...
    def run(self):
        ...
        for t, _ in enumerate(self.at):
            ...
            self.y += (-(self.a0 - self.at[t]) * self.x + self.c0 * self.coupling @ sign(self.x)) * self.delta_t
...
if __name__ == '__main__':
    ...
    dSB = DiscreteSimulatedBifurcation(position=x_position,
                                       momentum=y_momentum,
                                       epochs=args.epochs,
    ...
    energy_curve(solver=dSB)
    # energy_hist(solver=dSB, trials=100)
